"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ShippingInfoModule = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const shipping_info_service_1 = require("./shipping-info.service");
const shipping_info_resolver_1 = require("./shipping-info.resolver");
const shipping_info_entity_1 = require("./entities/shipping-info.entity");
const order_entity_1 = require("../orders/entities/order.entity");
const address_entity_1 = require("../addresses/entities/address.entity");
let ShippingInfoModule = class ShippingInfoModule {
};
exports.ShippingInfoModule = ShippingInfoModule;
exports.ShippingInfoModule = ShippingInfoModule = __decorate([
    (0, common_1.Module)({
        imports: [typeorm_1.TypeOrmModule.forFeature([shipping_info_entity_1.ShippingInfo, order_entity_1.Order, address_entity_1.Address])],
        providers: [shipping_info_resolver_1.ShippingInfoResolver, shipping_info_service_1.ShippingInfoService],
    })
], ShippingInfoModule);
//# sourceMappingURL=shipping-info.module.js.map